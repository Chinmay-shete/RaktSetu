  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (must_change_password) {
        localStorage.setItem('raktsetu_state_admin', JSON.stringify({ status: 'pending_password_reset', user }));
              ...
              <>
                ...
                {changeSuccess ? (
                  <div className="p-5 bg-green-50 border border-green-200 text-[#22A06B] rounded-2xl flex flex-col items-center gap-3 text-center">
                    ...
                    <p className="text-xs text-[#5A5A5A]">Logging you into RaktSetu State Dashboard...</p>
                  </div>
                ) : (
                  <>
                    {/* Error */}
                    ...
                    <form onSubmit={handlePasswordChangeSubmit} className="space-y-5">
                      ...
                      <button
                        type="submit"
                        disabled={isLoading}
                        className="btn-state w-full cursor-pointer"
                        style={{ minHeight: 52 }}
                      >
